...
//	private BorderLayout defaultLayout = new BorderLayout();
	...
	//	Yamel Senih
	//	Add for Fiscal Printer Login
	private CLabel fiscalPrinterLabel = new CLabel();
	private VComboBox fiscalPrinterField = new VComboBox();
	//	End
	...
	private void jbInit() throws Exception
	{
		...
		//	Fiscal Printer
		fiscalPrinterLabel.setRequestFocusEnabled(false);
		...
		defaultPanel.add(fiscalPrinterLabel,        new GridBagConstraints(0, 6, 1, 1, 0.0, 0.0
			,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 12, 12, 5), 0, 0));
		defaultPanel.add(fiscalPrinterField,        new GridBagConstraints(1, 6, 1, 1, 1.0, 0.0
			,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 12, 12), 0, 0));
		//	End
...
*/
	...
	private boolean defaultsOK ()
	{
		...
		//	Yamel Senih
		//	Fiscal Printer set Values
		KeyNamePair pair = (KeyNamePair) fiscalPrinterField.getSelectedItem();
		if(pair != null) {
			String fiscalPrinterName = pair.getName();
			//	Optional Printer
			if (fiscalPrinterName == null)
				fiscalPrinterName = "";
			//	Set
			Env.setContext(m_ctx, FiscalPrinterHandler.CTX_FISCAL_PRINTER_NAME, fiscalPrinterName);
			Env.setContext(m_ctx, FiscalPrinterHandler.CTX_FISCAL_PRINTER_ID, pair.getKey());
			Ini.setProperty(FiscalPrinterHandler.INI_FISCAL_PRINTER_NAME, fiscalPrinterName);
			Ini.setProperty(FiscalPrinterHandler.INI_FISCAL_PRINTER_ID, pair.getKey());
		}
		//	End
